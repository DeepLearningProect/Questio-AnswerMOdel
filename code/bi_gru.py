...
# tested with recurrent_cell_size = 256
...
# or other issues.
# tested with learning_rate = 0.001 
learning_rate = 0.005

# Dropout probabilities. For a description of dropout and what these probabilities are, 
# see Entailment with TensorFlow.
# tested with input_p, output_p = 0.7, 0.3
...
fw_cell=tf.contrib.rnn.GRUCell(64)
bw_cell=tf.contrib.rnn.GRUCell(64)

# input_p: The probability of maintaining a specific hidden input unit.
# Likewise, output_p is the probability of maintaining a specific hidden output unit.

fw_cell = tf.contrib.rnn.DropoutWrapper(fw_cell, input_p, output_p)
bw_cell = tf.contrib.rnn.DropoutWrapper(bw_cell, input_p, output_p)

# dynamic_rnn also returns the final internal state. We don't need that, and can
# ignore the corresponding output (_). 
#input_module_outputs, _ = tf.nn.dynamic_rnn(gru_drop, context, dtype=tf.float32, scope = "input_module")
input_module_outputs,input_module_outputs_bw = tf.nn.bidirectional_dynamic_rnn(fw_cell,bw_cell,context, dtype=tf.float32, scope = "input_module")
...
question_module_outputs,question_module_outputs_bw = tf.nn.bidirectional_dynamic_rnn(fw_cell,bw_cell, query, dtype=tf.float32, scope = tf.VariableScope(True, "input_module"))
